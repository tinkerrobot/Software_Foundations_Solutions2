...
(** Which of the following are well-typed elements of [grumble X] for
    some type [X]?
      - [d (b a 5)] <= X d 옆에 X에 해당 하는 것이 없어서.
      - [d mumble (b a 5)] <= O - mumble
      - [d bool (b a 5)] <= O - bool
      - [e bool true] <= O - bool
      - [e mumble (b c 0)] O - mumble
      - [e bool (b c 0)] X - (b c 0)가 bool이 아니므로
      - [c] X - 그냥 mumble 형임.
(* FILL IN HERE *)
 *)
...
(*   end. *)
   ...
   my comment : I think it's difficult for the beginner to solve so I was in troubes.
  걍 한글로 쓰자. split(과제 03 - P01 첫번째)와 split_map(과제 03 - P01 두번째) 문제가 나름 난이도가 있는듯. 나중에 다시 보자.
 **)
...
Fixpoint split {X Y : Type} (l : list (X*Y)) : (list X) * (list Y) :=
  match l with
  | [] => ([], [])
  | (h1, h2) :: t => app_list_pair ([h1], [h2]) (split t) end.
...
Proof.
  intros X Y Z f p. destruct p as [m n]. reflexivity.
Qed.
