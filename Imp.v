...
Proof.
  ...
  - (* BEq *) simpl. rewrite optimize_0plus_sound. rewrite optimize_0plus_sound. reflexivity.
  - (* BLe *) simpl. rewrite optimize_0plus_sound. rewrite optimize_0plus_sound. reflexivity.
    ...
    + apply E_WhileLoop with (t_update (t_update (t_update (t_update (t_update (t_update empty_state X 2) Y 0) Y 2) X 1) Y 3) X 0).
      ...
      * apply E_Seq with (t_update (t_update (t_update (t_update (t_update empty_state X 2) Y 0) Y 2) X 1) Y 3).
        { apply E_Ass. simpl. rewrite t_update_shadow. rewrite t_update_permute.
          ...
          - apply beq_id_false_iff. destruct (beq_id Y X) eqn: H.
            + inversion H.
            + reflexivity. }
        { apply E_Ass. simpl. reflexivity. }
...
Proof.
  ...
  (** Proceed by induction on the assumed derivation showing that
      ...
      [inversion]). *)
  induction contra.
  - (* SKIP *) inversion Heqloopdef.
  - (* ASS *) inversion Heqloopdef.
  - (* SEQ *) inversion Heqloopdef.
  - (* IfTrueB *) inversion Heqloopdef.
  - (* IfFalseB *) inversion Heqloopdef.
  - (* WhileEnd *) inversion Heqloopdef. rewrite H1 in H. inversion H.
  - (* WhileLoop *) inversion Heqloopdef. subst. contradiction.
Qed.
...
Lemma sinstr_div :
  forall (st : state) (stack : list nat) (s1 : list sinstr) (s2 : list sinstr) (n : nat) (l : list nat),
    s_execute st stack s1 = n :: l -> s_execute st stack (s1 ++ s2) = s_execute st (n :: l) s2.
Proof. Admitted.

Lemma stack_div :
  forall (st : state) (stack : list nat) (s : list sinstr) (n : nat) (l : list nat),
    s_execute st stack s = [n] -> s_execute st (stack ++ l) s = n :: l.
Proof. Admitted.
